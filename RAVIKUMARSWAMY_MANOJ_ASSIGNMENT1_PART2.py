...
Original file is located at
    https://colab.research.google.com/drive/10YYiVk_hLT_V7cj5vp0ePXixkE2PAwNb
...
def question_six():
    ...
    for token, freq in fdist.items():
        if token.isalpha() and freq > 2000:
            filtered_words.append((freq, token))
...
def question_nine():
    ...
    filtered_tokens = [token.lower() for token in moby_tokens if token.isalpha()]
...
def question_ten():
    ...
    # 2. Lowercase, 3. Keep only alphabetic tokens, 4. Remove stop words
    filtered_tokens = [token.lower() for token in tokens if token.isalpha() and token.lower() not in stop_words]
    # 5. Lemmatize
    preprocessed_tokens = [lemmatizer.lemmatize(token) for token in filtered_tokens]
